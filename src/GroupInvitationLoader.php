...
use Drupal\ginvite\Plugin\GroupContentEnabler;
...
class GroupInvitationLoader implements GroupInvitationLoaderInterface {
  ...
  protected function wrapGroupContentEntities($entities) {
    ...
    foreach ($entities as $group_content) {
      $group_invitations[] = new GroupInvitation($group_content);
    }
   ...
   * {@inheritdoc}
   */
  public function loadByGroup(GroupInterface $group, $roles = NULL, $mail = NULL, $status = GroupContentEnabler\GroupInvitation::INVITATION_PENDING) {
    $filters = [
   ...
   * {@inheritdoc}
   */
  public function loadByUser(AccountInterface $account = NULL, $roles = NULL, $status = GroupContentEnabler\GroupInvitation::INVITATION_PENDING) {
    if (!isset($account)) {
